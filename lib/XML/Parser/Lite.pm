...
#
# $Id: XML::Parser::Lite.pm,v 0.10 2001/04/18 11:45:14 $
#
# ======================================================================
...
# need to run 're' only where required and do not fail on old Perl's
sub compile { 

  no strict 'subs'; # hm, how else can I check this version?
  die __PACKAGE__, " requires Perl v5.6\n" unless $^V && $^V ge v5.6.0;

  # although in theory it might work in Perl 5.005 it doesn't for some reason
  # and results are unexpected and significantly different from Perl 5.6
  ...
  my $XML_regexp = do { 
    ...
    my $AttValSE = "\"([^<\"]*)\"|'([^<']*)'";
    my $ElemTagCE = "($Name)(?:$S($Name)(?:$S)?=(?:$S)?(?:$AttValSE)(?{[\@{\$^R||[]},\$4=>\$5||\$6]}))*(?:$S)?(/)?>(?{${package}::start(\$3,\@{\$^R||[]})})(?{\${7} and ${package}::end(\$3)})";
    my $MarkupSPE = "<(?:!(?:$DeclCE)?|\\?(?:$PI_CE)?|/(?:$EndTagCE)?|(?:$ElemTagCE)?)";
    my $XML_SPE = "($TextSE)(?{${package}::char(\$1)})|$MarkupSPE";
  };

  eval qq{sub parse_re { use re "eval"; 1 while \$_[0] =~ m{$XML_regexp}go }; 1} or die;
...
sub char { 
  ...
  # can't use split or regexp here due to limitations in (?{}) implementation, 
