...
my $myrand = sub { my $N = defined $_[0] ? $_[0] : 1;  rand($N); };
...
sub command_interface {
	...
	print "$self->{initial}->[ int &{$myrand}( scalar @{ $self->{initial} } ) ]\n";
	...
	while (1) {
		...
		if ($self->_testquit($user_input) ) {
			$reply = "$self->{final}->[ int &{$myrand}( scalar @{$self->{final}} ) ]";
			print $self->botprompt if $self->prompts_on;
			print "$reply\n";
			last;
		} 
		   ...
		   ) 
		{
			...
			DECOMP: foreach $decomp (@{ $self->{decomplist}->{$keyword} }) {
				...
				if ($string_part =~ /$this_decomp()()()()()()()()()()/i) {
					...
					$reasmb = $these_reasmbs[ int &{$myrand}( scalar @these_reasmbs ) ];
...
=cut
	...
	if ($reasmb eq "") {
		...
		if (
			$#{ $self->memory } >= 0 
			and 
			&{$myrand}(1) >= 1 - $self->likelihood_of_using_memory
		) {
